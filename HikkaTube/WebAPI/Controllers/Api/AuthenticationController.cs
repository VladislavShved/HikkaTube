...
namespace WebApi.Controllers.Api
{
    public class AuthenticationController : ApiController
    {
        ...
        public HttpResponseMessage Authenticate([FromBody]LoginModel model)
        {
            ...
            {
                var authUser = UserMapper.ToCoreModel(user);

                var response = new HttpResponseMessage(HttpStatusCode.OK);

                var js = new JavaScriptSerializer();
                var data = js.Serialize(authUser);
                var ticket = new FormsAuthenticationTicket(1, authUser.Login, DateTime.Now, DateTime.Now.AddMinutes(20), model.IsRemember, data);
